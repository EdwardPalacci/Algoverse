...
ROOT = repo_root()
OUTPUT_DIR = ROOT / "analysis" / "llm_as_judge" / "results"
...
def source_paths(source: str, raw=None, parsed=None, output=None) -> tuple[Path, Path, Path, Path]:
    ...
    output_root = ROOT / f"{source}_models" / "model_outputs"
    raw_path = Path(raw) if raw else output_root / "raw_by_model"
    parsed_path = Path(parsed) if parsed else output_root / "parsed_by_model"
    if not raw_path.exists() and not raw:
        raw_path = output_root / f"{source}_raw_generations.jsonl"
    if not parsed_path.exists() and not parsed:
        parsed_path = output_root / f"{source}_parsed_generations.jsonl"
    error_path = ROOT / f"{source}_models" / "logs" / f"{source}_run_errors.md"
    output_path = Path(output) if output else OUTPUT_DIR / source
...
def read_jsonl(path: Path) -> list[dict]:
    ...
    if not path.exists():
        return rows
    if path.is_dir():
        all_dataset_files = sorted(path.rglob("all_datasets.jsonl"))
        paths = all_dataset_files if all_dataset_files else sorted(path.rglob("*.jsonl"))
    else:
        paths = [path]
        ...
        with jsonl_path.open("r", encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, 1):
                ...
                try:
                    ...
                    raise ValueError(f"Invalid JSONL in {jsonl_path} line {line_number}: {exc}") from exc
    return rows
...
def write_split_results(source: str, rows: list[dict], output_root: Path, append: bool = False) -> None:
    if output_root.exists() and not append:
        shutil.rmtree(output_root)
    by_model: dict[str, list[dict]] = {}
    for row in rows:
        by_model.setdefault(str(row.get("model_name", "unknown_model")), []).append(row)

    for model_name, model_rows in sorted(by_model.items()):
        model_dir = output_root / "by_model" / safe_model_name(model_name)
        write_jsonl(model_dir / "all_datasets.jsonl", model_rows, append=append)

        by_dataset: dict[str, list[dict]] = {}
        for row in model_rows:
            by_dataset.setdefault(str(row.get("dataset", "unknown_dataset")), []).append(row)
        for dataset, dataset_rows in sorted(by_dataset.items()):
            write_jsonl(model_dir / "by_dataset" / f"{dataset}.jsonl", dataset_rows, append=append)
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.output:
        write_jsonl(output_path, rows_to_write, append=output_mode == "a")
    else:
        write_split_results(args.source, rows_to_write, output_path, append=output_mode == "a")
